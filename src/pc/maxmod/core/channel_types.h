...
#ifdef __GBA__
static_assert(sizeof(mm_module_channel) == MM_SIZEOF_MODCH, "mismatched MM_SIZEOF_MODCH");
#endif
...
#ifdef __GBA__
static_assert(sizeof(mm_active_channel) == MM_SIZEOF_ACTCH, "mismatched MM_SIZEOF_ACTCH");
#endif
...
#define MIXCH_GBA_SRC_STOPPED   ((uintptr_t)1u << ((sizeof(uintptr_t) * 8) - 1))

// Make sure that the size matches the assembly code
static_assert(sizeof(mm_mixer_channel) == 24, "invalid mm_mixer_channel size");
static_assert(sizeof(mm_mixer_channel) == MM_SIZEOF_MIXCH, "mismatched MM_SIZEOF_MIXCH");
